...
{
    public class MonkeysRowViewModel : INotifyPropertyChanged
    {
        static IList<Monkey> source;

        static MonkeysRowViewModel()
        {
            CreateSourceOfCollection();
        }

        public ObservableCollection<Monkey> Monkeys { get; private set; }

        public MonkeysRowViewModel(int start, int length)
        {
            Monkeys = new ObservableCollection<Monkey>();
            for (var i = start; i < start + length; i++)
            {
                Monkeys.Add(source[i]);
            }
            Position = 0;
        }
        ...
        static void CreateSourceOfCollection()
        {
